...
RESULTS_DIR = PROJECT_ROOT / "results" / "round04_path_a_tuned_vanilla_dqn"
...
def cache_path(tag: str) -> Path:
    return CACHE_DIR / f"{tag}__300000__seed11.json"
...
def run_trial(tag: str, label: str, config: dict) -> dict:
    ...
    callback = EvalCurveCallback(eval_env, CHECKPOINT_FREQ, N_EVAL_EPISODES)

    print(f"Running experiment | {tag} | budget=300000 | seed=11")
    model = DQN(seed=BASE_SEED, env=train_env, verbose=0, **config)
...
def export_plot(baseline_run: dict, tuned_run: dict) -> Path:
    fig, ax = plt.subplots(figsize=(10.4, 5.8))
    for run, linestyle in [(baseline_run, "--"), (tuned_run, "-")]:
    ...
    ax.set_title("Path A: baseline vs tuned vanilla DQN (300k)")
    ax.set_xlabel("Environment steps")
    ax.set_ylabel("Deterministic eval mean return")
    ax.grid(True, linestyle="--", alpha=0.35)
    ax.legend(loc="best", fontsize=9)
    out = PLOT_DIR / "baseline_vs_tuned_vanilla_dqn_300k.png"
...
def main() -> None:
    ensure_dirs()
    baseline_run = run_trial("baseline", "baseline DQN", BASELINE_CONFIG)
    tuned_run = run_trial("path_a_best", "Path A tuned vanilla DQN", PATH_A_BEST_CONFIG)
    plot_path = export_plot(baseline_run, tuned_run)
