...
SHAPING_CONFIGS = {
    "low": {
        "label": "objective shaping: low (0.3/0.3/0.3/0.05/0.05)",
        "weights": ObjectiveShapingWeights(stable=0.30, crash=0.30, timeout=0.30, abs_x=0.05, fuel=0.05),
    },
    "base": {
        "label": "objective shaping: base (0.5/0.5/0.5/0.1/0.1)",
        "weights": ObjectiveShapingWeights(stable=0.50, crash=0.50, timeout=0.50, abs_x=0.10, fuel=0.10),
    },
    "high": {
        "label": "objective shaping: high (0.7/0.7/0.7/0.1/0.1)",
        "weights": ObjectiveShapingWeights(stable=0.70, crash=0.70, timeout=0.70, abs_x=0.10, fuel=0.10),
    },
}
...
def trial_cache_path(tag: str) -> Path:
    if tag == "base":
        return ROUND06_OBJECTIVE_BASE_CACHE
    return CACHE_DIR / f"trial_baseline_plus_objective_shaping_{tag}__300000__seed11.json"


def run_trial(tag: str, label: str, weights: ObjectiveShapingWeights) -> dict:
    cp = trial_cache_path(tag)
    if cp.exists():
        print(f"Loading cache | objective_shaping_{tag} | budget=300000 | seed=11")
    ...
    print(f"Running experiment | objective_shaping_{tag} | budget=300000 | seed=11")
    model = AuxiliaryObjectiveShapingDQN(
        seed=BASE_SEED,
        env=train_env,
        verbose=0,
        auxiliary_loss_weights=AUX_WEIGHTS_015,
        objective_shaping_weights=weights,
        **DQN_BASELINE_CONFIG,
    )
    model.learn(total_timesteps=LONG_BUDGET, callback=callback, progress_bar=False)

    payload = {
        "name": f"trial_baseline_plus_objective_shaping_{tag}",
        "label": label,
        "seed": BASE_SEED,
        "budget": LONG_BUDGET,
...
def export_plot(control_run: dict, low_run: dict, base_run: dict, high_run: dict) -> Path:
    ...
    ax.set_title("Path C: objective shaping coefficient sweep (300k)")
    ax.set_xlabel("Environment steps")
    ax.set_ylabel("Deterministic eval mean return")
    ax.grid(True, linestyle="--", alpha=0.35)
    ax.legend(loc="best", fontsize=9)
    out = PLOT_DIR / "objective_shaping_coefficient_sweep_300k.png"
...
def main() -> None:
    ensure_dirs()
    control_raw = load_json_run(ROUND04_BASELINE_CACHE)
    control_run = {
        "name": "control_baseline_dqn",
        "label": "control: baseline DQN",
        "seed": BASE_SEED,
        "budget": LONG_BUDGET,
        "config": control_raw["config"],
        "records": control_raw["records"],
    }

    low_run = run_trial("low", SHAPING_CONFIGS["low"]["label"], SHAPING_CONFIGS["low"]["weights"])
    base_run = run_trial("base", SHAPING_CONFIGS["base"]["label"], SHAPING_CONFIGS["base"]["weights"])
    high_run = run_trial("high", SHAPING_CONFIGS["high"]["label"], SHAPING_CONFIGS["high"]["weights"])
    plot_path = export_plot(control_run, low_run, base_run, high_run)
