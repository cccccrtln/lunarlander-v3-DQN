...
from auxiliary_objectives import AuxiliaryFeedbackDQN, AuxiliaryLossWeights, LunarLanderAuxiliaryInfoWrapper
...
def trial_cache_path() -> Path:
    return CACHE_DIR / "trial_baseline_plus_aux_feedback_heads__300000__seed11.json"


def run_trial() -> dict:
    cp = trial_cache_path()
    if cp.exists():
        print("Loading cache | aux_feedback | budget=300000 | seed=11")
    ...
    print("Running experiment | aux_feedback | budget=300000 | seed=11")
    model = AuxiliaryFeedbackDQN(
        seed=BASE_SEED,
        env=train_env,
        verbose=0,
    ...
    payload = {
        "name": "trial_baseline_plus_aux_feedback_heads",
        "label": "trial: aux heads + Q feedback (0.15 each)",
        "seed": BASE_SEED,
        "budget": LONG_BUDGET,
...
def export_plot(control_run: dict, aux_run: dict, feedback_run: dict) -> Path:
    ...
    ax.set_title("Path C: baseline vs auxiliary heads vs auxiliary-informed Q (300k)")
    ax.set_xlabel("Environment steps")
    ax.set_ylabel("Deterministic eval mean return")
    ax.grid(True, linestyle="--", alpha=0.35)
    ax.legend(loc="best", fontsize=9)
    out = PLOT_DIR / "baseline_vs_aux_vs_aux_feedback_300k.png"
...
def main() -> None:
    ensure_dirs()
    control_run = load_json_run(ROUND04_BASELINE_CACHE)
    control_run = {
        "name": "control_baseline_dqn",
        "label": "control: baseline DQN",
        "seed": BASE_SEED,
        "budget": LONG_BUDGET,
        "config": control_run["config"],
        "records": control_run["records"],
    }
    aux_run = load_json_run(ROUND06_AUX015_CACHE)
    feedback_run = run_trial()
    plot_path = export_plot(control_run, aux_run, feedback_run)
