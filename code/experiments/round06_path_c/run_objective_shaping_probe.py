...
SHAPING_WEIGHTS = ObjectiveShapingWeights(
    stable=0.50,
    crash=0.50,
    timeout=0.50,
    abs_x=0.10,
...
def trial_cache_path() -> Path:
    return CACHE_DIR / "trial_baseline_plus_objective_shaping__300000__seed11.json"


def run_trial() -> dict:
    cp = trial_cache_path()
    if cp.exists():
        print("Loading cache | objective_shaping | budget=300000 | seed=11")
    ...
    print("Running experiment | objective_shaping | budget=300000 | seed=11")
    model = AuxiliaryObjectiveShapingDQN(
        seed=BASE_SEED,
        env=train_env,
        verbose=0,
        auxiliary_loss_weights=AUX_WEIGHTS_015,
        objective_shaping_weights=SHAPING_WEIGHTS,
        **DQN_BASELINE_CONFIG,
    )
    model.learn(total_timesteps=LONG_BUDGET, callback=callback, progress_bar=False)

    payload = {
        "name": "trial_baseline_plus_objective_shaping",
        "label": "trial: aux heads + objective shaping",
        "seed": BASE_SEED,
        "budget": LONG_BUDGET,
        ...
        "shaping_weights": {
            "stable": SHAPING_WEIGHTS.stable,
            "crash": SHAPING_WEIGHTS.crash,
            "timeout": SHAPING_WEIGHTS.timeout,
            "abs_x": SHAPING_WEIGHTS.abs_x,
            "fuel": SHAPING_WEIGHTS.fuel,
        },
...
def export_plot(control_run: dict, aux_run: dict, shaped_run: dict) -> Path:
    ...
    ax.set_title("Path C: baseline vs auxiliary heads vs objective shaping (300k)")
    ax.set_xlabel("Environment steps")
    ax.set_ylabel("Deterministic eval mean return")
    ax.grid(True, linestyle="--", alpha=0.35)
    ax.legend(loc="best", fontsize=9)
    out = PLOT_DIR / "baseline_vs_aux_vs_objective_shaping_300k.png"
...
def main() -> None:
    ...
    shaped_run = run_trial()
    plot_path = export_plot(control_run, aux_run, shaped_run)
