...
def structure_only_cache_path() -> Path:
    return CACHE_DIR / "trial_double_dqn_structure_only__300000__seed11.json"


def run_variant(name: str, label: str, overrides: dict) -> dict:
    cp = CACHE_DIR / f"{name}__300000__seed11.json"
    if cp.exists():
...
def load_structure_only_as(label: str) -> dict:
    payload = json.loads(structure_only_cache_path().read_text(encoding="utf-8"))
    payload["label"] = label
    return payload


def export_plot(runs: list[dict], title: str, filename: str) -> Path:
...
def main() -> None:
    ...
    ddqn_base = load_structure_only_as("Double DQN: baseline")

    maxgrad_runs = [
        run_variant("ddqn_mgn_1_0", "Double DQN: max_grad_norm=1.0", {"max_grad_norm": 1.0}),
        run_variant("ddqn_mgn_2_0", "Double DQN: max_grad_norm=2.0", {"max_grad_norm": 2.0}),
        run_variant("ddqn_mgn_5_0", "Double DQN: max_grad_norm=5.0", {"max_grad_norm": 5.0}),
        ddqn_base,
    ]
    plot1 = export_plot(
        maxgrad_runs,
        "Path B: Double DQN max_grad_norm retuning (300k)",
        "ddqn_retune_max_grad_norm_300k.png",
    )

    gradstep_runs = [
        ddqn_base,
        run_variant("ddqn_gs_2", "Double DQN: gradient_steps=2", {"gradient_steps": 2}),
        run_variant("ddqn_gs_4", "Double DQN: gradient_steps=4", {"gradient_steps": 4}),
    ]
    plot2 = export_plot(
        gradstep_runs,
        "Path B: Double DQN gradient_steps retuning (300k)",
        "ddqn_retune_gradient_steps_300k.png",
    )
