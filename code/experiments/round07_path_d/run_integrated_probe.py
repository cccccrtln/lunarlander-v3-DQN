...
def integrated_cache_path() -> Path:
    return CACHE_DIR / "trial_pathD_integrated__300000__seed11.json"


def run_integrated() -> dict:
    cp = integrated_cache_path()
    if cp.exists():
        print("Loading cache | path_d_integrated | budget=300000 | seed=11")
    ...
    print("Running experiment | path_d_integrated | budget=300000 | seed=11")
    ...
    payload = {
        "name": "trial_pathD_integrated",
        "label": "Path D: tuned + Double + objective shaping",
        "seed": BASE_SEED,
        "budget": LONG_BUDGET,
...
def export_plot(path_a: dict, path_b: dict, path_c: dict, integrated: dict) -> Path:
    ...
    ax.set_title("Round 07 Path D: integrating Path A + B + C (300k)")
    ax.set_xlabel("Environment steps")
    ax.set_ylabel("Deterministic eval mean return")
    ax.grid(True, linestyle="--", alpha=0.35)
    ax.legend(loc="best", fontsize=9)
    out = PLOT_DIR / "path_d_integrated_compare_300k.png"
...
def main() -> None:
    ensure_dirs()
    path_a = load_run(PATH_A_CACHE)
    path_b = load_run(PATH_B_CACHE)
    path_c = load_run(PATH_C_CACHE)
    integrated = run_integrated()
    plot_path = export_plot(path_a, path_b, path_c, integrated)
